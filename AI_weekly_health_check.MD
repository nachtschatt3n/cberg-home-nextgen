...
**IMPORTANT**: Replace `secret-domain` in all commands with your actual domain name. Your domain can be found in:
- `kubernetes/apps/kube-system/authentik/app/configmap.sops.yaml` (SOPS encrypted)
- `kubernetes/apps/network/external-dns/app/secret.sops.yaml` (SOPS encrypted)
- Or check your DNS provider for the configured domain

Example: If your domain is `mydomain.com`, replace `secret-domain` with `mydomain.com` in all commands.
...
- Verify Zigbee2MQTT coordinator connectivity and device health
- Monitor MQTT broker status, client connections, and message flow
- Check ESPHome device connectivity and firmware updates
- Review Node-RED flows for errors and performance
- Verify Scrypted camera feeds and motion detection
- Assess Zigbee network health, device battery levels, and message routing
...
|---------|----------------|-----------------|---------------|--------------|
| Authentik | ✅ | ✅ | Healthy | 2025-12-13 |
| Home Assistant | ✅ | ✅ | Healthy | 2025-12-13 |
| etc... | | | | |
...
## Standardized Health Check Output Schema

### Output Format Specification

The health check must produce output in the following standardized format:

```markdown
# Kubernetes Cluster Health Check Report
**Date**: YYYY-MM-DD
**Cluster**: cluster-name
**Nodes**: X (node1, node2, node3)
**Duration**: XmYs (time taken to run checks)

## Executive Summary
- **Overall Health**: [✅ Excellent|🟡 Good|🟠 Warning|🔴 Critical]
- **Critical Issues**: X (issues requiring immediate attention)
- **Warnings**: X (issues requiring attention this week)
- **Service Availability**: XX% (healthy/total services)
- **Uptime**: [All systems operational|Partial issues|Major outages]

## Service Availability Matrix
| Service | Internal | External | Health | Response | Status Notes |
|---------|----------|----------|--------|----------|--------------|
| ServiceName | ✅/❌/⚠️ | ✅/❌/⚠️ | [Healthy/Degraded/Failed] | [Time/N/A] | [Brief status] |

## Detailed Findings

### 1. Cluster Events & Logs
✅ **Status: [OK|Warning|Critical]** - Brief description
- Metric: X events in last 7 days
- Issues: [List any issues found]
- Recommendation: [Action if needed]

### 2. Jobs & CronJobs
✅ **Status: [OK|Warning|Critical]** - Brief description
- Active CronJobs: X (list names)
- Last backup: YYYY-MM-DD HH:MM:SS
- Failed jobs: X in last 7 days

[... continue for all 30 sections with consistent format ...]

## Performance Metrics
- **Average Response Times**: X.XXs across Y tested services
- **Resource Utilization**: CPU X%, Memory Y% average across Z nodes
- **Network Performance**: X clients, Y packet loss, Z Mbps throughput
- **Database Load**: PostgreSQL X connections, MariaDB Y queries/sec
- **Error Rate**: [Low/Medium/High] (X errors in last 24h)

## Version Report
| Component | Current | Latest | Status | Priority | Notes |
|-----------|---------|--------|--------|--------|----------|
| ComponentName | vX.Y.Z | vA.B.C | [Up-to-date|Update Available|Critical] | [High/Med/Low] | [Additional info] |
...
- Task 1: Description and benefit
- Task 2: Description and benefit
...
## Trends & Observations
- **Resource Usage**: [Stable/Increasing/Decreasing] - current levels and trends
- **Performance**: [Consistent/Degrading/Improving] - key metrics and changes
- **Reliability**: [Excellent/Good/Fair/Poor] - uptime and error rates
- **Capacity**: [Good/Monitor/At Risk] - current usage vs limits
- **Security**: [Clean/Suspicious Issues] - authentication and access patterns

---
**Report Generated**: YYYY-MM-DD HH:MM:SS
**Health Check Version**: v2.0 (30 sections)
**Next Scheduled Check**: YYYY-MM-DD (weekly)
```

### Status Indicator Standards

#### Overall Health Levels:
- **✅ Excellent**: 0 critical issues, 0-1 warnings, 100% service availability
- **🟡 Good**: 0 critical issues, 2-5 warnings, 95-99% service availability
- **🟠 Warning**: 1-2 critical issues, 6+ warnings, 90-94% service availability
- **🔴 Critical**: 3+ critical issues, major service outages, <90% availability

#### Section Status Indicators:
- **✅ OK**: No issues, all metrics within acceptable ranges
- **⚠️ Warning**: Issues detected but not critical, monitoring recommended
- **❌ Critical**: Immediate action required, potential data loss or service impact

#### Service Health Status:
- **Healthy**: Service responding normally, all checks passed
- **Degraded**: Service operational but with performance issues
- **Failed**: Service not responding or critical functionality broken
- **Unknown**: Unable to determine status (check failed)
...
- **Low**: <50% (good headroom)
- **Moderate**: 50-75% (monitor trends)
- **High**: 75-90% (capacity planning needed)
- **Critical**: >90% (immediate action needed)

#### Error Rates:
- **Low**: <1% or <10 errors/day
- **Medium**: 1-5% or 10-100 errors/day
- **High**: >5% or >100 errors/day

## Expected Output Example

The health check should produce output exactly matching this standardized format:
...
- Zigbee2MQTT: Coordinator connected, 13 active devices, 8 offline devices detected
...
- **Zigbee Issues:**
  - **Offline devices (5+ days):** Entry Door, Living Room Window, Entry Temp Sensor, Guest toilet Temp sensor, SLZB-06 Router
  - **Never seen devices (3):** Yard Soil Sensor Roses, Yard Soil Sensor 2, Yard Soil Sensor 3
  - **Battery levels:** Excellent (100%: 21 devices, 97%: 9 devices, 90%: 3 devices)
  - **Link Quality:** Variable (4-153), some devices may need repositioning
- **Recommendations:** Check battery levels, reposition weak signal devices, investigate never-seen sensors
...
- **Zigbee Performance**: 13 active devices, 8 offline devices, link quality 4-153, battery levels 90-100%
...
- **Troubleshoot 8 offline Zigbee devices:**
  - Check battery levels on Entry Door, Living Room Window, Entry Temp Sensor, Guest toilet Temp sensor
  - Reposition SLZB-06 Router for better connectivity
...
**🤖 AI-Optimized Diagnostics (Recommended):**
```bash
...
for switch in devices:
    ...
    if poe_ports:
        ...
        if poe_budget > 0 and total_poe_power/poe_budget > 0.9:
            print(f'    ⚠️  HIGH PoE USAGE (>90%) - consider load balancing or higher budget switch')
        elif poe_budget > 0 and total_poe_power/poe_budget > 0.8:
            print(f'    ℹ️  Moderate PoE usage (>80%)')
        ...
        if poe_errors:
            print(f'    ⚠️  PoE Faults on {len(poe_errors)} port(s):')
            for p in poe_errors:
...
for switch in devices:
    ...
    if error_ports:
        print(f'\\n  {name}:')
        print(f'    ⚠️  {len(error_ports)} port(s) with errors:')
...
for switch in devices:
    ...
    print(f'\\n  {name}:')
    if stp_blocking:
        print(f'    ⚠️  {len(stp_blocking)} port(s) in BLOCKING state (preventing loops):')
...
for switch in devices:
    ...
    if uplink_port:
        ...
        if speed < 1000:
            print(f'    ⚠️  Slow uplink (<1Gbps) - check cable or port')
        elif not full_duplex:
            print(f'    ⚠️  Half-duplex detected - check cable or switch port')
        elif speed >= 10000:
            print(f'    ✅ 10GbE uplink')
    else:
...
for switch in devices:
    ...
    if utilization > 90:
        print(f'    ⚠️  High port utilization (>90%) - consider expansion')
    elif utilization > 80:
        print(f'    ℹ️  Moderate port utilization (>80%)')
"
```
...
if 'satisfaction' in data:
    ...
    if score < 80:
        print(f'  ⚠️  LOW SATISFACTION - investigate interference or coverage')
    elif score < 90:
        print(f'  ℹ️  Moderate satisfaction - room for improvement')
    else:
        print(f'  ✅ Excellent satisfaction')
else:
...
for ap in devices:
    ...
    for radio in radios:
        ...
        flag = ''
        if utilization > 70:
            flag = ' ⚠️  HIGH INTERFERENCE'
        elif utilization > 50:
            flag = ' ℹ️  Moderate usage'
    ...
    if scan:
        for band, data in scan.items():
            ...
            if neighbors:
                ...
                if strong_neighbors:
                    print(f'    ⚠️  {len(strong_neighbors)} strong interfering APs (signal > -70 dBm)')
"
    ...
    for row in matrix:
        for col in matrix[row]:
            ...
            if score < 30 and row != col:  # Low isolation between different radios
                issues.append(f\"  ⚠️  Low isolation ({score}) between {row} and {col}\")
    if issues:
        print('\\n'.join(issues))
        print('\\n  Action: Consider adjusting AP placement or channel selection')
    else:
        print('  ✅ Good isolation between all radios')
else:
...
for band, aps in channels.items():
    ...
    for ap, ch in aps:
        flag = ''
        if ch_count[ch] > 1:
            flag = f' ⚠️  {ch_count[ch]} APs on same channel (potential interference)'
...
for ap in aps:
    ...
    if clients > max_clients:
        flag = f' ⚠️  HIGH LOAD (>{max_clients} for {wifi_gen})'
    elif clients == 0:
        flag = ' ℹ️  No clients'
    elif is_wifi7 and clients > 30:
        flag = f' ✅ Good load for {wifi_gen}'
    elif clients > max_clients * 0.7:
        flag = f' ℹ️  Moderate load for {wifi_gen}'
...
if total > 0:
    ...
    if band_24/total > 0.8:
        print(f'  ⚠️  Very high 2.4 GHz usage ({band_24/total*100:.0f}%) - verify IoT devices')
        print(f'      Check if modern devices are stuck on 2.4 GHz instead of 5/6 GHz')
    elif band_6 > 0:
        print(f'  ✅ WiFi 6E/7 devices utilizing 6 GHz band')
"
```
...
# Check MQTT broker connections and status
kubectl exec -n home-automation deployment/mosquitto -- netstat -tlnp | grep :1883
kubectl exec -n home-automation deployment/mosquitto -- mosquitto_sub -t '$SYS/broker/clients/connected' -C 1 -W 2 2>/dev/null || echo "MQTT client count check failed"

# Check Zigbee2MQTT coordinator status and device health
kubectl logs -n home-automation deployment/zigbee2mqtt --tail=20 | grep -i "coordinator\|connected\|error"
kubectl logs -n home-automation deployment/zigbee2mqtt --tail=50 | grep -i "device.*joined\|device.*left\|interview\|failed" | tail -10

# Check total configured devices vs active devices
kubectl exec -n home-automation deployment/zigbee2mqtt -- cat /data/configuration.yaml | grep -c "friendly_name:"
kubectl exec -n home-automation deployment/zigbee2mqtt -- cat /data/state.json | jq 'length'

# Check Zigbee device last seen timestamps (identify offline devices)
kubectl exec -n home-automation deployment/zigbee2mqtt -- cat /data/state.json | jq -r 'to_entries[] | select(.value.last_seen) | "\(.key): \(.value.last_seen)"' | while read line; do device=$(echo "$line" | cut -d: -f1); timestamp=$(echo "$line" | cut -d: -f2- | sed 's/^ *//'); days_diff=$(( ($(date +%s) - $(date -d "$timestamp" +%s)) / 86400 )); if [ $days_diff -gt 1 ]; then friendly_name=$(kubectl exec -n home-automation deployment/zigbee2mqtt -- grep -A1 "$device" /data/configuration.yaml | grep "friendly_name" | cut -d: -f2 | sed 's/^ *//'); echo "$friendly_name ($device): $days_diff days ago"; fi; done

# Check for devices configured but never seen
kubectl exec -n home-automation deployment/zigbee2mqtt -- sh -c "
echo 'Devices configured but never seen:'
for device in \$(grep '0x' /data/configuration.yaml | grep -v friendly_name | cut -d: -f1 | sed \"s/ *'//;s/'.*//\"); do
  if ! grep -q \"\$device\" /data/state.json 2>/dev/null; then
    friendly_name=\$(grep -A1 \"\$device\" /data/configuration.yaml | grep friendly_name | cut -d: -f2 | sed 's/^ *//')
    echo \"\$friendly_name (\$device)\"
  fi
done
"

# Check Zigbee2MQTT device count and battery levels
kubectl logs -n home-automation deployment/zigbee2mqtt --tail=200 | grep -o '"battery":[0-9]*' | sort | uniq -c | sort -nr | head -5

# Check Zigbee2MQTT network health and LQI (Link Quality Indicator)
kubectl logs -n home-automation deployment/zigbee2mqtt --tail=100 | grep -o '"linkquality":[0-9]*' | sort | uniq -c | sort -nr | head -5

# Check ESPHome devices and firmware versions
kubectl get pods -n home-automation -l app.kubernetes.io/name=esphome -o jsonpath='{.items[*].status.phase}'
kubectl logs -n home-automation -l app.kubernetes.io/name=esphome --tail=20 | grep -i "version\|update\|firmware" | tail -5

# Check Node-RED flows and performance
kubectl logs -n home-automation deployment/node-red --tail=10 | grep -i "error\|started\|stopped\|deployed"
kubectl exec -n home-automation deployment/node-red -- node-red-admin list 2>/dev/null | wc -l 2>/dev/null || echo "Node-RED flow count check failed"

# Check Scrypted camera status
kubectl logs -n home-automation deployment/scrypted --tail=10 | grep -i "camera\|motion\|recording\|error" | tail -5

# Monitor MQTT message flow (subscribe to test topic)
kubectl run mqtt-test --rm -it --image=alpine --restart=Never -- apk add mosquitto-clients && mosquitto_sub -h mosquitto.home-automation.svc.cluster.local -t 'zigbee2mqtt/#' -C 3 -W 5 2>/dev/null | wc -l | xargs echo "MQTT messages received:" || echo "MQTT message flow test failed"
```
...
for domain in "auth.secret-domain" "hass.secret-domain" "drive.secret-domain"; do
  echo "Testing $domain:"
...
curl -s -w "Time: %{time_total}s\n" -o /dev/null https://auth.secret-domain
curl -s -w "Time: %{time_total}s\n" -o /dev/null https://hass.secret-domain
```
...
```bash
# ✅ Check subsystem status with proper interpretation
unifictl local health get -o json | python3 -c "
import sys, json
health = json.load(sys.stdin)
...
if [ $NON_RUNNING -gt 0 ]; then
    echo "ALERT: $NON_RUNNING pods not in running state" | mail -s "Cluster Health Alert" admin@secret-domain
fi
```
