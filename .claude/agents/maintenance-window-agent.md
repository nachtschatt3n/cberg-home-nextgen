...
risk-load vs capacity. **Non-safe updates are operator-go/no-go by default**
(`execution.default_mode`). Only proceed unattended for a plan with
`auto_execute: true` AND `risk: low` AND `execution.unattended_allowed: true`.
Otherwise wait for explicit operator approval of the sequence (they may approve a
subset).
...
3. Run its **Verification**. If it fails → run its **Rollback** immediately
   (revert + confirm restore), mark the plan `blocked`, alert, and STOP the
   sequence (do not start the next plan on a degraded cluster).
...
the failure), deferred (with the window they moved to), and the remaining
held-update backlog. Emit an `auto-update`/`maintenance` finding to the sweep DB
if anything blocked.
