...
- If a fix requires state changes (delete/restart/reconcile/apply), ask for approval first — AND emit the recommendation as a FINDING (P4.1.6): a recommendation that exists only in report prose does not exist. Findings get lanes, SLAs and reminders; report text dies with the report. Emit via FindingsWriter with an `action` naming the exact state change and its owner.
...
- Zigbee2MQTT — `docs/sops/zigbee2mqtt.md`. The runbook script now queries `bridge/devices` + `bridge/info` and alerts on: missing Coordinator (critical), Router count below baseline (major), any device with `interview_completed=false` (major — usually a CC2652-router ZDO bug; the SOP §4d has the DB-injection recovery), or `permit_join=True` at sweep time (minor). Do not propose firmware reflash or radio Mode-toggle as the first recovery for an interview-failing SLZB router — DB injection per the SOP is the proven path.
