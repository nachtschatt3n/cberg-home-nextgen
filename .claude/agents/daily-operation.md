...
0. **First, mint ONE shared sweep cycle id — all specialists write to it.**
   ...
   **Propagate the run trigger too.** If the invoking prompt declares a run
   trigger — e.g. it contains a line like
   `[AUTOMATED SCHEDULED RUN — SWEEP_TRIGGER=cron]` or otherwise tells you to
   set `SWEEP_TRIGGER` — then in ADDITION to `export SWEEP_CYCLE_ID=<id>`, put
   `export SWEEP_TRIGGER=<that value>` (e.g. `cron`) in EVERY specialist's
   prompt, right alongside the cycle-id export. The check scripts read it via
   `findings_writer.trigger_from_env()` and stamp `sweep_cycles.trigger` (the
   first specialist to write the row wins and it is never overwritten), so an
   autonomous cron run records `trigger='cron'` instead of the default
   `'manual'`. If the prompt declares no trigger (an operator-initiated run),
   do nothing extra — it correctly defaults to `'manual'`.
