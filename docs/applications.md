...
| wazuh-agent | DaemonSet — one privileged Wazuh agent per cluster node, enrolled with stable identity `k8s-nuc14-{01,02,03}` (pinned via `WAZUH_AGENT_NAME=$(NODE_NAME)` so DaemonSet rollouts don't create zombie agent IDs; manager `<auth><purge>yes</purge>` reuses the same ID on re-enrollment). Collects FIM (rootcheck on `/etc /usr/bin /usr/sbin /bin /sbin /boot`), Talos node logs (kubelet, kernel, machined, containerd, cri), and Kubernetes pod stdout via `/host/var/log/containers/*.log` (every CRI log on the node, except `wazuh-*` to avoid feedback loops). 4.14.5, AR-023 + AR-025. | None | — |
