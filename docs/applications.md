...
> only to back one app (`penpot-db`, `superset-pg18`, `paperless-db`,
...
| mcpo | Model Control Plane Orchestrator | None — cluster-only Service; no HTTPRoute exists | AI |
| ai-sre | AI-powered SRE tooling | None — cluster-only Service; no HTTPRoute exists | AI |
...
| superset | Apache Superset — data exploration and visualization. **Auth model — read before touching credentials or rolling back:** the intended front door is **Authentik SSO via OIDC** (`AUTH_TYPE = AUTH_OAUTH`), but that setting does **not** disable Flask-AppBuilder's local `db` provider. `/api/v1/security/login` advertises `db` unconditionally (an unknown provider returns `Must be one of: db, ldap`), so **a valid db-provider password grants full Superset Admin while bypassing Authentik entirely** — no OIDC, no MFA, no Authentik audit trail. A `401` on that endpoint means "wrong password", never "no password path" — the general rule and the credential-free provider-enumeration check are in `docs/sops/sso-local-auth-bypass.md`. Consequently: **`init.createAdmin` is `false`** (set `dad8922c`) so the chart no longer (re)asserts a local admin on every upgrade, and admin credentials live **only in SOPS** (`secret.sops.yaml`) — never as a literal in a values file, this repo being public. **ROLLBACK CAVEAT — mandatory pre-condition:** the frozen `superset-pg` (17.11) backup set predates the 2026-09-08 rotation and therefore carries the *pre-rotation* FAB password hashes for **every** local db-provider Admin account (currently **two**: `admin` and `mu_adm`), and Helm history Secrets `v58`–`v61` likewise carried the pre-rotation rendered manifest — **those are now GONE** (`v58`/`v59` aged out of Helm's `maxHistory: 5`; `v60`/`v61`, which held `init.createAdmin: true` plus the full `adminUser` block, were deleted 2026-09-09), so **the Helm-history half of this caveat is CLOSED, and since 2026-09-11 the volume half is closed too — what remains is the **backup** half, now narrowed to a SINGLE retained `superset-pg-data` Longhorn backup (the other six were deleted 2026-09-11)**. **Restoring that data** — a **restore-from-backup** operation since 2026-09-11, when the volume itself was reclaimed (see below): there is no running `superset-pg` Deployment or Service to repoint at *and* no retained PVC left to bind, so recovery means restoring the single surviving Longhorn backup `backup-5876963a2bce454c` (SINGLE-COPY since 2026-09-11 — there is no second copy and no fallback) into a new volume and re-creating that Deployment from git history (`git show 9d10199c^:kubernetes/apps/databases/superset/app/pg-deployment.yaml`) — re-arms a credential whose plaintext was public for ~4.7 months and can never be un-published. **Any rollback across that boundary MUST reset the password for EVERY local db-provider account holding Admin — currently `admin` (rotated `dad8922c`, SOPS key `ADMIN_PASSWORD`) and `mu_adm` (rotated 2026-09-08 `0af4b8e5`, SOPS key `MU_ADM_PASSWORD`) — or disable those FAB users, as part of the rollback itself, before Superset serves traffic**. Enumerate accounts with `superset fab list-users` in-pod rather than assuming a single admin: provider enumeration alone cannot reveal how many accounts hold a usable local password — not as follow-up work. The same applies to any restore from a backup taken before 2026-09-08. Image **`apache/superset:6.1.0`**, upgraded from `5.0.0` on 2026-09-08 (plan `superset-6.1.0`: two majors of one-way alembic migrations plus a Flask-AppBuilder major — there is no image-tag rollback, only a DB restore). Metadata DB runs on the standalone **`superset-pg18`** Deployment + `superset-pg18` Service — Docker Official `postgres:18.6-alpine` (OCI-index digest-pinned), plain manifests in `pg18-deployment.yaml`/`pg18-pv.yaml`/`pg18-pvc.yaml`, on the `longhorn-static` volume `superset-pg18-data` (its Longhorn `Volume` CR, `pg18-longhorn-volume.yaml`, is hand-applied and deliberately out of `kustomization.yaml`). **`PGDATA` is pinned explicitly to `/var/lib/postgresql/data/pgdata`** — load-bearing, because `postgres:18` moved its default PGDATA and VOLUME; without the pin the datadir lands in the container's ephemeral layer while every ordinary check still passes (`docs/sops/postgres-major-upgrade.md`). **SQL Lab Redis auth:** chart 0.22.4 builds `RESULTS_BACKEND` from a Helm *value* (`cache.password`) while its siblings `CACHE_CONFIG`/`DATA_CACHE_CONFIG` read `env('REDIS_PASSWORD')` at runtime; we leave that value unset on purpose (public repo), so it rendered `password=""` and every SQL Lab query 500'd with "Authentication required." from 2026-08-17 to 2026-09-09. Fixed in `e351c722` via `values.config.resultsBackend`, the chart's raw-string escape hatch, which expresses the same runtime lookup. Cut over 2026-09-08 (plan `superset-pg-18.6`, ~4 min outage; 53/53 tables, 50 sequences, 127 indexes and 111 foreign keys diffed identical before the repoint, restart assertion passed twice). **`superset-pg` (`postgres:17.11-alpine`) — WORKLOAD RETIRED 2026-09-09 (`9d10199c`), DATA RETAINED.** The Deployment and Service are deleted (`pg-deployment.yaml` removed; it was idle, 0 client backends at teardown). `pg-pv.yaml`/`pg-pvc.yaml`/`pg-longhorn-volume.yaml` and the 20Gi `longhorn-static` volume `superset-pg-data` were kept for a time as a deliberately-orphaned rollback artifact, and were then **RECLAIMED 2026-09-11** by plan `superset-pg-decommission` — the manifests are deleted from the app folder and the PVC, PV and Longhorn `Volume` CR are gone from the cluster. That plan **IS** the "separate, explicitly-decided cleanup" this entry previously deferred to, so the earlier "do NOT treat it as an orphan to reclaim" instruction is **superseded, not ignored**. Its frozen Longhorn backup set was **TRIMMED TO A SINGLE BACKUP on 2026-09-11** (§7.2 of the plan — the "further step" this entry previously called still-undecided; it is now decided and executed). **The entire pre-pg18 restore path is now ONE object with no second copy anywhere: `backup-5876963a2bce454c`** (2026-09-09T03:04:08Z, 922,746,880 B, `Completed`). The six older dailies (09-03…09-08) were deleted: they predate the 5.0.0→6.1.0 alembic migration and carry the 5.0.0 schema, so they were restorable only alongside an image downgrade that does not exist — deleting them cost no rollback capability. They also predate the 2026-09-08 rotation, which is why they were not kept indefinitely on a share with no at-rest encryption. Verified on volume-reclaim day by restoring the surviving backup into a scratch volume and booting it: its `alembic_version` matched the live `superset-pg18` value exactly — **note that proof predates the sibling deletions** (`F-059aca4a`), so a fresh `runbooks/backup-restore-proof.py` drill is the outstanding confirmation that the surviving block set is complete. **Do NOT delete `BackupVolume/superset-pg-data-26df02ea`** — the surviving backup is owned by it and would cascade. **EXPIRY: retire this backup on or after 2026-12-08** (a full quarter on 6.1.0/pg18); until then it is the only pre-cutover metadata rollback that exists. That 17.11 instance was itself the 2026-08-19 cutover target (plan `superset-pg-cutover`, 47 tables / 2985 rows verified identical before the repoint). The chart-bundled bitnamilegacy PostgreSQL 14.17 was **retired 2026-09-05** (`90539942`, stage 4 `superset-pg-decommission`, after a 17-day soak): `postgresql.enabled: false` removed the `superset-postgresql` StatefulSet + Services + SA + PDB + NetworkPolicy. **`database.host: superset-pg18` and `cache.host: superset-redis-official` are pinned in the HelmRelease** because the chart's `wait-for-postgres`/`wait-for-redis` init containers read ONLY the chart-generated `superset-env` Secret, whose `DB_HOST`/`REDIS_HOST` default to `<release>-postgresql`/`<release>-redis-headless` — unset, every pod restart blocks 120 s on the dead host and exits 1. The DB pin landed as its own commit (`c4694b13`) ahead of the decommission, deliberately — and had to be moved AGAIN to `superset-pg18` in `f297f4b5`, because the 2026-09-08 cutover commit (`d9863640`) changed only `DB_HOST` in `secret.sops.yaml`, which reaches the MAIN containers and not the chart-generated `superset-env` the init gate reads. Both env sources must move in the same change set; see `docs/sops/container-dependencies.md`. Its data (the 20Gi `longhorn-static` volume `superset-postgresql-data`, plus `pv.yaml`/`data-pvc.yaml`/`longhorn-volume.yaml`) was retained at the time as a deliberately-orphaned rollback artifact, and was **RECLAIMED 2026-09-11** by plan `superset-pg-decommission` — the explicitly-decided cleanup this entry previously deferred to. Manifests deleted, PVC/PV/Longhorn `Volume` CR removed. **Its frozen backup set NO LONGER EXISTS — all seven were deleted 2026-09-11 (§7.2 of the plan). Do not go looking for a fallback here; there is none.** They were obsolete twice over: postgres 14.17 carrying a Superset ≤5.0.0 schema has no viable restore into today's 6.1.0 app, so the set had already been written off as unrecoverable before it was deleted. Its `BackupVolume/superset-postgresql-data-b3f40529` record remains but is empty (`lastBackupName: ""`, `dataStored: 0`). Cache/broker on the standalone `superset-redis-official` Deployment — official `redis:8.10.1-alpine`, bundled bitnamilegacy Redis retired 2026-08-17 (`cache.host` is set for the same init-container reason). | Internal | Databases |
| sweep-history | Postgres holding the operator-curated policy tables (`accepted_risks`, `slo_definitions`, `noise_suppressions`, `security_acceptances`) plus sweep findings/cycles and, since 2026-09-23, `media_audit_runs` (written by the `library-tools` audit CronJob's `persist` container). Edited via `runbooks/policy-cli.py`. | None | Databases |
---
...
| App | Purpose | Ingress | Homepage Group |
|-----|---------|---------|---------------|
| authentik | Identity provider + forward auth proxy. Core DB runs on the standalone `authentik-pg` Deployment + Service — Docker Official `postgres:18.6-bookworm`, plain manifests in `pg-deployment.yaml`/`pg-pv.yaml`/`pg-pvc.yaml`, on the `longhorn-static` volume `authentik-pg-data` (20Gi; its Longhorn `Volume` CR, `pg-longhorn-volume.yaml`, is hand-applied and deliberately out of `kustomization.yaml`). **`max_connections=500` must match what the bundled DB was configured with** — authentik's server+worker replicas open far more than the postgres default 100, so a parity miss surfaces as login failures under load rather than at cutover. The chart-bundled PostgreSQL 17.11 rollback (Docker Official `postgres:17.11-bookworm` in the chart's bitnami-style layout) (StatefulSet `authentik-postgresql`) was retired 2026-09-24 by plan `authentik-pg17-decommission`; its PV and Longhorn volume `data-authentik-postgresql-0` were deleted 2026-09-26 by plan `authentik-pg17-volume-retire`; its 9 Longhorn backups (BackupVolume `data-authentik-postgresql-0-6dd5bdc0`, newest `authentik-pg17-decommission-preflight-20260926`, restore-proven 2026-09-26) are kept as a frozen pre-cutover (2026-08-19) copy — never restore them to recover current authentik data. Always query `deploy/authentik-pg`; see `docs/sops/authentik.md` §"Two databases answered to `-U authentik -d authentik`". | **External** — `authentik-server` on `envoy-external`. The many `*-authentik-outpost` forward-auth routes are `envoy-internal` and belong to the apps they gate, not to authentik itself | System |
...
| external-dns | `network/external/` | Automated Cloudflare DNS record management — the ONLY writer of public DNS for `${SECRET_DOMAIN}`. Runs **`policy: sync`**, which DELETES a record before recreating it, so a rejected create is a full public-DNS outage rather than a no-op (94 s on 2026-09-08). Sources `crd` + `gateway-httproute`, scoped default-deny to the single Gateway `envoy-external`; the record target comes from the **Gateway** annotation, never from the HTTPRoute. **Deny-listed from unattended auto-update at every update type** (`runbooks/auto-update-policy.yaml`) because v0.22.x needs `--default-targets` in the same change. Alerts: `kubernetes/apps/monitoring/kube-prometheus-stack/app/external-dns-alerts.yaml`. SOP: `docs/sops/external-dns.md`. | None |
...
The two `icloud-docker` instances are one per Apple ID; they share nothing but
the `csi-driver-smb` credential and the image digest. Coverage of non-Drive/Photos iCloud data (contacts,
...
- [ ] Add `gethomepage.dev/*` annotations **and** the `gethomepage.dev/enabled` label to the app's **HTTPRoute** — Homepage runs `kubernetes.gateway: true` / `ingress: false`, so annotations on an Ingress register nothing, silently (see `docs/sops/homepage-integration.md`)
