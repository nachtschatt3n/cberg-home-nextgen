...
| `coverage.py` | **the same `[:3]` truncation, in a second, independent implementation** — found the same afternoon by asking why a freshly-surfaced Plex build bump sat in no lane. `_is_strictly_newer` and `_semver_type` both parsed with `re.findall(r"\d+", v)[:3]`, so two builds differing only in the 4th field compared EQUAL and `parse_actionable()` dropped the row. `_is_strictly_newer` is not just a downgrade suppressor, it is the DENOMINATOR: an item that never enters the enumeration can never be reported as a crack, so `covered: YES ✅ (no cracks)` printed over a real, actionable, unplanned update. Fixed with full-component parsing + zero-padding, keeping the immich `v3.1.0 → v1.116.0` phantom-downgrade guard intact (2026-08-24, `runbooks/tests/test-coverage-version-arity.py`). When you fix a comparator, grep the whole `runbooks/` tree for its twin | pass (silent drop rendered as full coverage) |
...
| `health-check.sh` | **coverage gap, found by the thing it should have caught.** Nothing in the sweep or in the alert rules watched a container walking into its OWN cgroup limit — memory was measured per NODE only, and Prometheus has 105 rule groups with 8 memory rules, none of them container-vs-its-limit. frigate leaked 5683 → 8078 MiB against an 8Gi limit over seven days; it never OOMKilled, because the main process is not what trips the cgroup, so `restarts: 0` and the section's own "No OOM kills" assertion both stayed green while its ffmpeg children died with ENOMEM and camera detect threads dropped. Total sweep signal: six log lines scored MINOR. The discriminator is the interesting half — LEVEL ALONE IS NOT THE SIGNAL, because penpot-frontend sits flat at 98.5% of a 512Mi limit forever on reclaimable page cache, and a ratio-only assertion would have made that a permanent unclearable MAJOR (the anti-pattern three rows above). Requires level AND 24h growth, reports its examined-count as the control, and keys the finding on `namespace/container` rather than the pod, whose ReplicaSet hash is alphanumeric and so survives the fingerprinter's digit-stripping (2026-08-24, `runbooks/tests/test-container-memory-headroom.py`). **Known residual blind spot, recorded in the code rather than left to be rediscovered:** a leak that SATURATES stops growing — the kernel reclaims under cgroup pressure instead of OOMKilling — so its curve becomes indistinguishable from the benign plateau and this check goes quiet while the container is still pinned at its ceiling and still failing child allocations. frigate was entering that regime as this shipped. The separating signal is the ENOMEM evidence, not the memory curve; correlating at-limit WITH §34's allocation-failure count is the follow-up | fail (a leak with a deadline, invisible to every existing check) |
...
| `health-check.sh` | section 35 enumerated **zero** Ingress objects after the ingress-nginx deletion and still printed "All ingress backends healthy" — a denominator of nothing scored as a pass, rule 1 (`0ad29c62`) | pass |
...
If a bad classifier already auto-closed findings, reopen them — findings are
data, and a wrong resolution is a data error, not just a display bug:

```sql
UPDATE sweep_findings SET resolved_at = NULL, status = 'unchanged'
WHERE finding_id IN (...);
```

**There is no `'open'` status.** The column's values are `new` / `unchanged` /
`resolved`, and "open" is the DERIVED set `status <> 'resolved'` (what
`render-board.py` counts). Writing `status = 'open'` invents a value nothing
