...
| `superset-pg` image `17.11-alpine → 18.6-alpine` | `deployment/superset-pg18` has served Superset's metadata DB since the **2026-09-08 cutover** (`d9863640`). The 17.11 pin in `pg-deployment.yaml` governs only `deployment/superset-pg`, deliberately LEFT RUNNING as the rollback under an operator-directed no-soak sequencing override, and still holding the pre-cutover data. Applying this bump migrates the rollback's data directory — destroying the safety net while the thing it protects is healthy. Note the second-order trap: `pg18-deployment.yaml` in the SAME folder carries a legitimate 18.6 pin, so a reader seeing both can easily "reconcile" them the wrong way. | A `superset-pg-decommission` plan, **not yet written** — owed after ≥7 clean days on `superset-pg18`, same shape as `authentik-pg17-decommission`. That plan must also move `values.database.host` off the rollback *before* retiring it (already done in `f297f4b5`) and dispose of the still-Bound `superset-postgresql-data` PVC. Until it lands, this row stays a phantom. |
