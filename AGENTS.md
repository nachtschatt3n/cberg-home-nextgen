...
Blueprints follow a per-app pattern where each app manages its own blueprint:
...
- **ConfigMap**: `authentik-blueprints` in `kube-system` namespace (generated by Kustomize)
- **Mount path**: `/blueprints` (automatically loaded by Authentik)

**Important**: Each app's blueprint file lives in the app's directory. When you remove an app, its blueprint is removed too, preventing orphaned authentication config.
