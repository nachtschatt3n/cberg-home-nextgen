...
1. **No CIFS/SMB/NFS PVC deletes without 3-step pre-flight.** Inspect `spec.csi.volumeAttributes.subdir`, `spec.persistentVolumeReclaimPolicy`, and the StorageClass before the action. If `subdir` is `/`/empty/`..`-traversed: **STOP — regardless of `reclaimPolicy`.** (The old rule also required `reclaimPolicy == Delete`, which made it unreachable: every CIFS class here is `Retain`, so a root-subdir PVC fell through to "safe". `subdir` is structural; `reclaim` is one `kubectl edit sc` from catastrophic.) Patch the PV to `Retain` first, or surface to the user with inventory and ask explicit go/no-go.
2. **"Tear down the Job + PVC" is not routine for shared-fs PVCs.** Blast radius is set by the StorageClass, not the brief.
3. **All 19 CIFS classes are `reclaimPolicy: Retain` (re-verified live 2026-09-20)**, so a PVC delete today leaves data intact. The tiers below are blast radius **if reclaim were ever `Delete`**. **Catastrophic** (would wipe the whole shared export): `cifs-jellyfin-media`, `cifs-plex-media` — the only two with `subdir: /`, on `//NAS/media`. **Severe** (would wipe one app's data): `cifs-frigate-media`, `cifs-scrypted-media`, `cifs-icloud-docker-{mu,andrea}`, `cifs-jdownloader-media`, `cifs-makemkv-media`, `cifs-tube-archivist-media`, `cifs-nextcloud-data`, `cifs-paperless-{consume,export,inbox,log,media}`, `cifs-immich-cache`, `cifs-immich-icloud-backup`, `cifs-opencode-andreamosteller`, `cifs-penpot-assets`. **`cifs-immich-icloud-backup` is broader than it looks**: its subdir is `icloud-backup`, the PARENT of both `icloud-backup/mu` and `icloud-backup/andrea`, so it spans BOTH Apple IDs' backups rather than one app's data — it is mounted `ro` by both consumers, which is the mitigation, not the tier. This list is all 19 (2 catastrophic + 17 severe); if it ever names fewer than the live count, treat the gap as unassessed rather than safe. Full table with sources/subdirs in `docs/sops/storage-safety.md`.
...
Document management (paperless-ngx: Epson ES-580W scanner → SMB inbox → validator → consume pipeline, email ingestion, native AI (LLM suggestions + RAG, retired the paperless-gpt/paperless-ai sidecars 2026-08-24), OCR tuning `deu+eng`/`force`/rotate, library curation):
- `docs/sops/paperless.md`
- Sub-agent: `paperless-agent` (global, `~/.claude/agents/paperless-agent.md`) — full ingestion-pipeline context, hard rules (OCR_MODE=force ⇒ 6Gi/OOM, inline-PDF email gotcha, rotate-both-tesseract for flipped pages, native AI is DB-stored config not GitOps), "check paperless" health snapshot. Delegates cluster/PVC/manifest changes to cberg-agent/cluster-ops-agent, Ollama models to ollama-agent, scanner PoE/VLAN to unifi-agent.
...
- **EVERY Kubernetes outpost MUST set `kubernetes_disabled_components: [ingress]` — including the `managed:` system outposts that have no blueprint file.** Otherwise Authentik's outpost controller publishes its OWN `Ingress` holding the app's hostname on the default ingress class — an object that exists in no git repo and carries no ownerRefs, so it is invisible to a repo grep and survives deleting the app's own routing. It has mis-routed a hostname three times, once costing 40 minutes on headlamp. Deleting that Ingress does not hold (the outpost recreates it); disabling the component does. See `docs/sops/authentik.md` §"Outpost-published Ingress". **Audit from the LIVE outpost list, never from a repo grep**: `rg kubernetes_disabled_components` enumerates the outposts we declared, not the outposts that exist. The `authentik Embedded Outpost` is created by authentik's own code (`authentik/outposts/apps.py`), had no blueprint, sat on the `Local Kubernetes Cluster` service connection with `[]`, and was invisible to every previous audit — a latent hostname hijack that only its 0 providers made harmless. It is now blueprinted (`embedded-outpost-blueprint.yaml`, bound on `managed:`); anything else with a Kubernetes service connection and `disabled=[]` must be fixed while its provider count is still 0. Re-check after every authentik upgrade — a code-created object can change its defaults with no signal in this repo. Audit command and the three preconditions for safely blueprinting a managed object: `docs/sops/authentik.md` §"The rule covers MANAGED/SYSTEM outposts too".
...
runbooks/                        # Recurring operational procedures + their scripts
  ...
  tests/                         # Regression tests for runbook scripts (run directly with python3)
docs/
