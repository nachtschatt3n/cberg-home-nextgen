...
3. **All 19 CIFS classes are `reclaimPolicy: Retain` (verified 2026-08-26)**, so a PVC delete today leaves data intact. The tiers below are blast radius **if reclaim were ever `Delete`**. **Catastrophic** (would wipe the whole shared export): `cifs-jellyfin-media`, `cifs-plex-media` — both `subdir: /` on `//NAS/media`. **Severe** (would wipe one app's data): `cifs-frigate-media`, `cifs-scrypted-media`, `cifs-icloud-docker-{mu,andrea}`, `cifs-jdownloader-media`, `cifs-makemkv-media`, `cifs-tube-archivist-media`, `cifs-nextcloud-data`, `cifs-paperless-{consume,export,inbox,log,media}`, `cifs-immich-cache`. Full table with sources/subdirs in `docs/sops/storage-safety.md`.
