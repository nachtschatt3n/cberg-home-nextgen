...
## Network Diagnostics with unifictl

### Overview
The cluster's network infrastructure is managed by UniFi equipment. Use `unifictl` for comprehensive network diagnostics, monitoring, and management.

### Configuration
```bash
# Configure unifictl once (credentials stored in /home/mu/code/unifictl/.unifictl.yaml)
...
# Monitor specific AP client count
...
### Integration with Kubernetes Network Issues

When troubleshooting Kubernetes network issues:
1. **Check UniFi gateway health**: `unifictl local health`
2. **Verify VLAN connectivity**: `unifictl local networks` (check k8s-network VLAN 55)
3. **Check switch uplinks**: `unifictl local devices --filter "SW"`
4. **Monitor cross-VLAN routing**: `unifictl local firewall-rules` (verify rules allow k8s ↔ other VLANs)
5. **Check mDNS bridging**: Verify VLANs in mDNS scope (documented in AGENTS.md)

### Output Formats
- `unifictl local <command>` - Pretty table (default)
- `unifictl local <command> -o json` - JSON for scripting
- `unifictl local <command> -o csv` - CSV for reporting
- `unifictl local <command> --watch 5` - Live refresh every 5 seconds
